...
그리고 백업을 잘 안해두는 편이라서 자동으로 내가 설정한 시간에 백업이 이루어지도록 하는 것은 정말 유용하다고 생각되었다.     
사용자 계정을 여러개 한꺼번에 만드는 작업을 할때는 정말 실습실 조교가 된거 같았고 언젠가 편리하게 사용할 수 있을거라고 생각되었다.    
```
**아쉬웠던 점**      
```
프로그램이 조금 길어지면 이해하는데 시간이 걸렸지만 그래도 긴 코드를 나누서 실습해보니까 이해하면서 실습에 임할 수 있었다.      
```
